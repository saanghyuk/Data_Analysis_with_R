...
matplot(FreeThrows)
FreeThrows
matplot(t(FreeThrows), type="b", pch=15:18, col=c(1:4, 6))
legend("bottomleft", inset=0.01, legend=Players, col=c(1:4, 6), pch=15:18, horiz=F)

FreeThrowAttempts
Games
matplot(FreeThrowAttempts, type="b", pch=15:18, col=c(1:4, 6))
legend("bottomleft", inset=0.01, legend=Players, col=c(1:4, 6), pch=15:18, horiz=F)



sample <- FreeThrowAttempts[1:3, , drop=F]

Games

hwplot <- function(data, rows=1:10){
  Data <- data[rows, , drop=F]
  matplot(t(Data), type="b", pch=15:18, col=c(1:4, 6))
  legend("bottomleft", inset=0.01, legend=Players[rows], pch=15:18, col=c(1:4, 6))
}
#FreeThrow Attemps Per Game
hwplot(FreeThrowAttempts/Games)
#Accuracy of Free Throws
hwplot(FreeThrows/FreeThrowAttempts)

#Player playing style (2 vs 3 points preference) excluding Free Throws
hwplot((Points - FreeThrows) / FieldGoals)
